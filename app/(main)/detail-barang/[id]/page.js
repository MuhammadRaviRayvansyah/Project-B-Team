...
export default function DetailBarangPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [dataBarang, dataSemuaReview, dataSemuaBarang] = await Promise.all([
          getDetailBarang(id),
          getReview(),
          getBarang()
        ]);
  ...
  const avgRating = useMemo(() => {
    ...
    const total = ulasanBarang.reduce((sum, r) => sum + Number(r.rating || 0), 0);
  ...
  const totalHarga = useMemo(() => calculateTotal(), [tanggalPinjam, tanggalKembali, jumlah, barang]);
  ...
  if (isLoading) return <div className="text-center py-20">Memuat detail barang...</div>;
  if (!barang) return <div className="text-center py-20 font-bold text-red-500">Barang tidak ditemukan.</div>;
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row min-h-[500px]">
        ...
        <div className="md:w-1/2 p-8 flex flex-col">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-extrabold text-slate-900 mb-2">{barang.nama_barang}</h1>
            
            <div className="flex items-center gap-2 mb-4">
              <RatingStars rating={avgRating} size={18} />
              <span className="text-xs text-slate-500 font-medium">
                {ulasanBarang.length > 0 ? `${avgRating.toFixed(1)} (${ulasanBarang.length} Ulasan)` : "Belum ada ulasan"}
              </span>
            </div>

            <div className="flex items-end gap-2 mb-4">
              <span className="text-2xl font-bold text-emerald-600">Rp {(barang.harga_sewa || 0).toLocaleString("id-ID")}</span>
              <span className="text-sm font-medium text-slate-500 pb-1">/ hari</span>
            </div>
            <p className="text-sm text-slate-600 leading-relaxed">{barang.deskripsi}</p>
            
            <div className="mt-4 flex items-center gap-2">
              <span className="text-xs font-bold text-slate-500">Ukuran:</span>
              <span className="px-2.5 py-1 border border-slate-200 rounded text-xs font-bold text-slate-700">{barang.ukuran || "-"}</span>
            </div>
          </div>

          <div className="mt-auto pt-6 border-t border-slate-100">
            <h3 className="font-bold text-slate-900 mb-4">Form Pengajuan Peminjaman</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {totalHarga > 0 && (
                <div className="flex justify-between items-center py-3 border-t border-slate-100 mt-2">
                  <span className="text-sm font-bold text-slate-700">Total Biaya:</span>
                  <span className="text-lg font-bold text-emerald-600">Rp {totalHarga.toLocaleString("id-ID")}</span>
                </div>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
            {barangLain.map((item) => {
              const ulasanItem = semuaReview.filter(
                (r) => String(r.id_barang) === String(item.id_barang || item.id)
              );
              const itemAvgRating = ulasanItem.length ? ulasanItem.reduce((sum, r) => sum + Number(r.rating || 0), 0) / ulasanItem.length : 0;
                  ...
                  <div className="flex flex-col flex-1 py-1 pr-2">
                    ...
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      <span className="text-[10px] text-slate-500 font-medium">
                        {ulasanItem.length > 0 ? `(${ulasanItem.length})` : "Belum dinilai"}
                      </span>
                    </div>
                    <div className="mt-auto">
                      <p className="text-sm font-bold text-emerald-600">
                        Rp {(item.harga_sewa || 0).toLocaleString("id-ID")}
                        <span className="text-[10px] text-slate-500 font-medium"> / hari</span>
                      </p>
                    </div>
                  </div>
                </Link>

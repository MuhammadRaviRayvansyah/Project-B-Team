...
export default function ReviewPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const userName = currentUser.nama || currentUser.user?.nama || currentUser.name || `User ${userId}`;
    ...
    try {
      ...
      setErrorMsg("Gagal mengirim ulasan. Pastikan Anda sudah pernah meminjam barang ini.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f7f9ff] pb-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 flex flex-col gap-8">
        <Hero
          category="PENILAIAN"
          title="Review Barang"
        ...
        {isLoading ? (
          <div className="text-center py-20 font-medium text-slate-500">Memuat data...</div>
        ) : (
          <div className="flex flex-col gap-8">
            <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200/60 shadow-sm">
              <h3 className="text-lg font-bold text-slate-900 mb-6">Tulis Ulasan Baru</h3>
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-2">Pilih Barang</label>
                    <select
                      value={selectedId}
                      onChange={(e) => {
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-2">Bintang Penilaian</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-2">Komentar / Ulasan</label>
                  <textarea 
                    required
                    rows="3"
                    value={komentar}
            ...
            <div className="bg-white border border-slate-200/60 rounded-2xl shadow-sm overflow-hidden">
              <div className="p-6 border-b border-slate-100 bg-slate-50/30">
                <h3 className="text-lg font-bold text-slate-900">Data Ulasan Seluruh Pengguna</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="text-sm">
                    {reviewList.length === 0 ? (
                      <tr><td colSpan="4" className="text-center py-12 text-slate-500">Belum ada ulasan yang terdaftar.</td></tr>
                    ) : (
                      reviewList.slice().reverse().map((rev, index) => (
                        <tr key={rev.id_review || rev.id || index} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                          <td className="px-6 py-4 font-bold text-slate-800">
                            {getNamaBarang(rev.id_barang)}
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex flex-col">
                              <span className="text-slate-700 font-bold">{rev.nama_user || rev.nama || `User ${rev.id_user}`}</span>
                              <span className="text-[11px] text-slate-500 mt-0.5">{rev.tanggal_review || "Baru saja"}</span>
                            </div>
                          </td>
                          <td className="px-6 py-4">

...
export default function ReviewList({ id_barang, ulasan = [], onAddUlasan }) {
  ...
  return (
    <div>
      <button type="button" onClick={() => setOpen((prev) => !prev)} className="text-xs font-semibold text-slate-700 hover:underline flex items-center gap-1">
        ...
        <div className="mt-2">
          {ulasan.length === 0 ? (
            ...
            <div className="space-y-3">
              {ulasan.map((u, i) => (
                <div key={u.id_review || i} className="text-xs border-b border-slate-100 pb-2">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-slate-900">{u.nama_user || u.nama || `User #${u.id_user}`}</span>

export default function ItemCard({ nama, ukuran, stok, hargaPerHari, img, onDetail }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm flex flex-col overflow-hidden transition-shadow hover:shadow-md">
      <div className="relative w-full aspect-[4/3] bg-[#ebeef3] overflow-hidden">
        <img className="w-full h-full object-cover" src={img} alt={nama} />
        <div className="absolute top-[0.5rem] right-[0.5rem]">
          <span
            className={
              tersedia
                ? "bg-white/95 text-[#181c20] px-[0.5rem] py-[0.25rem] rounded text-[11px] leading-[14px] tracking-[0.02em] font-semibold flex items-center gap-1 shadow-sm"
                : "bg-white/95 text-[#181c20] px-[0.5rem] py-[0.25rem] rounded text-[11px] leading-[14px] tracking-[0.02em] font-semibold flex items-center gap-1 shadow-sm"
            }
          >
            <span className={tersedia ? "w-2 h-2 rounded-full bg-[#16A34A]" : "w-2 h-2 rounded-full bg-[#DC2626]"} />
      ...
      <div className="p-[1rem] flex flex-col flex-1 justify-between">
        <div>
          <div className="flex items-start justify-between gap-[0.5rem] mb-[0.25rem]">
            <h3 className="text-[16px] leading-[24px] font-semibold text-[#181c20]">{nama}</h3>
        ...
        <div className="pt-[0.5rem]">
          <button
            ...
            className={
              tersedia
                ? "w-full bg-[#2f3a4a] text-white hover:opacity-90 text-[14px] font-medium py-[0.5rem] rounded-lg transition-colors flex items-center justify-center gap-[0.5rem] h-10"
                : "w-full bg-[#e0e3e8] text-[#9aa0a6] cursor-not-allowed text-[14px] font-medium py-[0.5rem] rounded-lg flex items-center justify-center gap-[0.5rem] h-10"
            }
          >
